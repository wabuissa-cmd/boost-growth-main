...
import { useAuth, canEditStaffRequests, canManageLeaves } from "../auth";
...
export default function StaffLeave() {
  ...
  const showLeave = canManageLeaves(user);

  const availableTabs = useMemo(
    () => TABS.filter(t => (t.id === "staff" ? showStaff : showLeave)),
    [showStaff, showLeave]
  );
  ...
  return (
    <div>
      ...
      {activeTab === "balance" && showLeave && <LeaveBalance embedded />}
    </div>
  );
}
